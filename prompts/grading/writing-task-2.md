...
Every criterion `justification` must point to a **specific phrase** from the candidate's response, and the `evidence` field must contain that phrase. The `evidence` value is a plain JSON string holding **one** short verbatim excerpt — do **not** wrap it in quotation marks, do **not** put any double quote (`"`) inside it, and do **not** combine multiple excerpts (never write `'…' and '…'`). Use single quotes only if you must. No generic feedback like good range of vocabulary — point to the exact words.
