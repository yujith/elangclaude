...
### Reading: individual parts + full paper (ADR-0022)

One passage is still one `Test` (ADR-0003). A passage's **IELTS part** rides in `body_json` (`part?: 1|2|3`, stamped at generation for Academic; GT derives it from `gt_context` via `readingPart()` in `@elc/ai`). Learners filter the Academic Reading picker by part. A **full Reading paper** is `ReadingPaper` (global content) → three `ReadingPaperPart` slot rows pointing at approved passage-`Test`s. SuperAdmin curates a paper from 3 approved passages (→ Approved immediately) or generates a fresh 3-passage set (→ Draft until each part is approved) at `/content/reading/papers` (`apps/web/lib/reading/paper-actions.ts`; pure assembly rules = `validateCuration` in `@elc/ai`). Learners take a paper in one sitting via the "Full paper" tab → `ReadingPaperSession` (tenant-scoped) parents 3 part `Attempt`s, reusing the single-passage runner; `submitReadingAttempt` routes paper legs back to the orchestrator. The combined band sums the parts' raw counts through `bandFromPartial`. The 4-section **Full Mock's Reading leg IS a paper sitting** (`ReadingPaperSession.mock_session_id`): `ensureMockReadingPaper` creates a mock-linked sitting, part attempts carry both `mock_session_id` and `reading_paper_session_id`, and `readMockState` reads the Reading section state from the sitting's status (graded only when the whole paper is `Submitted`). Standalone resume queries filter `mock_session_id: null`. **End-to-end manual QA of the mock Reading leg is still pending.**
